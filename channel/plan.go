...
type Plan struct {
	...
	Discounts map[string]float32 `json:"discounts" mapstructure:"discounts"`
}
...
func IncreaseSubscriptionUsage(cache *redis.Client, user globals.AuthLike, t string, limit int64) (bool, int64, int64) {
	key := globals.GetSubscriptionLimitFormat(t, user.HitID())
...
func (p *PlanItem) Increase(user globals.AuthLike, cache *redis.Client) (bool, int64, int64) {
	state, before, after := IncreaseSubscriptionUsage(cache, user, p.Id, p.Value)
	if !state && !p.IsInfinity() {
