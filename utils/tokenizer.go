...
func GetWeightByModel(model string) int {
	...
	default:
		...
		} else if strings.Contains(model, "gpt-4.5") || strings.Contains(model, "gpt-4.1") || strings.Contains(model, "gpt-4o") {
			// newer gpt-4 variants (gpt-4.5, gpt-4.1, gpt-4o) — assume same weight as gpt-4
			return GetWeightByModel(globals.GPT40613)
...
func NumTokensFromMessages(messages []globals.Message, model string, responseType bool) (tokens int) {
	...
	if err != nil {
		// try a sensible fallback for newer models that use the cl100k_base encoding
		// e.g. newer chat models like gpt-5-* likely use the same encoding as gpt-3.5/gpt-4
		if globals.DebugMode {
			globals.Debug(fmt.Sprintf("[tiktoken] error encoding messages: %s (model: %s), trying cl100k_base encoding", err, model))
		}

		// attempt to get the cl100k_base encoding directly
		tkmFallback, err2 := tiktoken.GetEncoding(tiktoken.MODEL_CL100K_BASE)
		if err2 == nil {
			tkm = tkmFallback
		} else {
			// fallback failed, log and use the original default-model fallback
			if globals.DebugMode {
				globals.Debug(fmt.Sprintf("[tiktoken] fallback encoding cl100k_base failed: %s (model: %s), using default model instead", err2, model))
			}
